...
//
// Clicked from the magic-link email. Validates the single-use token, then opens
// (or updates) the submission's pull request via the GitHub API and redirects the
// author to the confirmation page.
//
// One submission per email: the first confirmation opens a PR; later confirmations
// from the same email UPDATE that same PR instead of opening a second one.
//
...
module.exports = async (req, res) => {
  ...
  if (!token) return redirect(res, '/submit/confirmed/?error=missing');
  ...
  try {
    ...
    return redirect(res,
      `/submit/confirmed/?status=${result.status}&pr=${encodeURIComponent(result.url)}`);
  } catch (e) {
    console.error('github error', e);
    return redirect(res, '/submit/confirmed/?error=github');
  }
};
...
async function createSubmission(sub) {
  ...
  // a branch may linger from a closed PR — make the name unique if so
  let finalBranch = branch;
  if (await branchExists(finalBranch)) finalBranch = `${branch}-${crypto.randomBytes(3).toString('hex')}`;
...
async function updateSubmission(sub, existing) {
  ...
  // get current file SHA on the existing branch (required to update it)
  let sha = null;
  let branchOk = true;
  try {
    ...
    branchOk = false; // branch or file gone (PR merged/deleted) — fall back to a fresh PR
  }
  ...
  // Fallback: previous PR is gone — open a fresh one.
  const fresh = await createSubmission(sub);
  return fresh;
}
...
function baseSlug(sub) {
  // first author surname, else title
